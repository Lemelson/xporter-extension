...
- On macOS under `CODEX_SANDBOX`, never invoke bare `soffice` or `node scripts/test-extension-smoke.mjs`: full LibreOffice/Chromium app binaries abort during LaunchServices registration. `node scripts/test-extension-core.js` resolves the bundled headless runtime automatically; for standalone conversions use `node scripts/soffice-headless.js ...`. Run the Playwright smoke only through an approved unsandboxed exec (`sandbox_permissions=require_escalated`) or a normal Terminal session.
