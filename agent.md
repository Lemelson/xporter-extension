...
>
> Last verified against the **v1.6.5** release runtime (2026-09-06); the `v1.6.4` tag is the Chrome Web Store release.
...
| Version | 1.6.5 (`manifest.json`), packaged 2026-09-06 |
...
│   ├── test-all.js              # Canonical ordered 12-suite deterministic gate
│   ├── test-extension-core.js   # 99-test aggregator; suites in test-extension-core/
...
- **Live request capture**: `content/interceptor.js` captures sanitized operation/queryId/features/field-toggle templates (plus `SearchTimeline` bodies) from X's own traffic. `utils/capture-contract.js` supplies the immutable operation/size limits in MAIN and via the generated `content/capture-prerequisites.js` bundle in the isolated world; isolated content and the worker/API apply their own sender and payload validation.
...
| `postSafetyBreakEnabled` / `postSafetyBreakMin` / `postSafetyBreakEvery` | `false` / `3` / `20` | optional Posts/Bookmarks scheduled break |
| `userSafetyBreakEnabled` / `userSafetyBreakMin` / `userSafetyBreakEvery` | `false` / `3` / `20` | optional User Lists scheduled break |
...
- **`utils/capture-contract.js`** loads first in MAIN (the generated `content/capture-prerequisites.js` bundle loads first in the isolated world) and exposes frozen tracked-operation and payload-size limits. It contains no credentials or mutable authority.
...
6. Completion reasons: `limit_reached` for the selected quantity; `source_exhausted` after a valid explicit bottom termination or the captured cursor-only empty-page shape (one `TimelineAddEntries` or two matching `TimelineReplaceEntry` instructions containing exactly the typed Top and Bottom cursor entries); `no_matches` when such an ending has no accepted rows. The `cursor-bottom-0` ID also appears on nonterminal pages and is never sufficient alone. Missing cursors, unknown endings and silence remain resumable errors. GraphQL errors and alerts are rejected; retries must repair the same request URL before later pages can be accepted. Only the initial response or a response for the expected Bottom request cursor can advance the export; Top or unrelated cursor responses are ignored. Resume restarts the search and deduplicates saved rows. Stop remains stopped. This describes X's returned search results, not proof that X indexed every historical post.
7. Persist `dateFromCalendar` and `dateToCalendar` alongside UTC bounds so current, multipart and history download names retain the selected dates across timezone changes. Missing display fields are filled from the resolved profile only after row author identity is verified.
8. Popup and overlay show loading, connecting, waiting, collecting and retrying phases. Delayed overlay initialization uses the current phase, and newer status revisions invalidate old delivery retries. Date-depth progress is approximate, capped at 99% while running and display-only. Short localized errors distinguish unavailable/changed tabs, a missing capture bridge, absent responses, invalid/error responses, oversized bodies, network failures and stalled pagination. Saved partial rows remain downloadable via **Download N rows**; Resume opens a new search tab.
...
**Manifest content worlds** (`document_start`): `capture-contract.js` → `native-request-template.js` → `feed-parser.js` → `interceptor.js` in MAIN; `content/capture-prerequisites.js` → `content.js` in the isolated world. The bundle combines the two canonical prerequisite sources under a distinct resource path because Chromium deduplicates script paths across worlds. Regenerate with `node scripts/check-capture-bundle.js --write`; `test-all.js` and packaging enforce byte-for-byte freshness.
